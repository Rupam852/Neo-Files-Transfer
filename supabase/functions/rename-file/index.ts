...
serve(async (req) => {
  ...
  try {
    ...
    const renameInDrive = async (token: string) => {
      return await fetch(
    ...
    let driveResponse = await renameInDrive(accessToken)
    ...
    if (driveResponse.status === 401) {
      ...
      if (profile.google_refresh_token) {
        try {
          accessToken = await refreshGoogleToken(user.id, profile.google_refresh_token, supabaseAdmin)
          driveResponse = await renameInDrive(accessToken)
        } catch (refreshError) {
          console.error("Token refresh failed during rename retry:", refreshError)
        }
      }
    }
